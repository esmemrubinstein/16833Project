...
def landmark_management(ekf_state):
    ...
    if num_landmarks > 0:
        ...
        # Remove landmarks with low utility
        threshold = np.percentile(landmark_utility, 5)
...
def run_ekf_slam(events, ekf_state_0, vehicle_params, filter_params, sigmas):
    ...
    for i, event in enumerate(events):
        ...
        if event[0] == 'gps':
            ...
            if ekf_state['num_landmarks'] > 300:
                ekf_state = landmark_management(trees, assoc, ekf_state, count)
    ...
    print("Number of landmarks", ekf_state['num_landmarks'])
...
def main():
    ...
    # save state history pose to file
    np.savetxt("pose.txt", state_history['x'])

    end_time = time.time()
    elapsed_time = end_time - start_time
    output_file = "program_times.txt"
