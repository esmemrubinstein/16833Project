...
def do_final_plot(xhist, ekf_state, trees, scan, assoc, plot, params):
    slam_utils.plot_trajectory(xhist, plot)

    plt.draw()
    plt.pause(50)

def extract_landmarks(scan, bearings):
    '''
    Loads the bearings from another .txt file and compiles full landmark measurement tuples from the
    given laser scan and frame idx (ignoring diameter because that is discarded anyway)
    '''

    measurements = []
    for r, b in zip(scan, bearings):
        measurements.append(np.array([r, b]))
    return np.array(measurements)

# TODO: motion model may need to be tweaked to work with KITTI data
...
def run_ekf_slam(events, ekf_state_0, vehicle_params, filter_params, sigmas):
    bearings = slam_utils.read_data_file("/home/ethan/coursework/16833/16833Project/kitti_data/{}_{}_bearing.txt".format(DATE, DRIVE))
    ...
    do_final_plot(state_history['x'], ekf_state, trees, scan, assoc, plot, filter_params)

    return state_history


def main():
    odo = slam_utils.read_data_file("/home/ethan/coursework/16833/16833Project/kitti_data/{}_{}_odo.txt".format(DATE, DRIVE))
    gps = slam_utils.read_data_file("/home/ethan/coursework/16833/16833Project/kitti_data/{}_{}_gps.txt".format(DATE, DRIVE))
    laser = slam_utils.read_data_file("/home/ethan/coursework/16833/16833Project/kitti_data/{}_{}_range.txt".format(DATE, DRIVE))
    ...
    # TODO: vehicle parameters may need to be tweaked to work with KITTI data
    vehicle_params = {
        "a": 3.78,
        "b": 0.50,
        "L": 2.83,
