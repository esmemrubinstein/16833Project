...
def initialize_landmark(ekf_state, tree):
    ...
    # Extract current covariance matrix and size
    P = ekf_state['P']
    n = ekf_state['num_landmarks']

    # Construct G_xi and G_z matrices
    G_xi_R = np.array([
        [1, 0, -m_r * np.sin(m_th + phi)],
        [0, 1,  m_r * np.cos(m_th + phi)]
    ])

    G_z = np.array([
        [np.cos(m_th + phi), -m_r * np.sin(m_th + phi)],
        [np.sin(m_th + phi),  m_r * np.cos(m_th + phi)]
    ])

    # Correct Q_kxi matrix construction
    I = np.eye(3 + 2 * n)
    Q_kxi = np.block([
        [I, np.zeros((I.shape[0], 2))],
        [np.zeros((2, I.shape[1])), G_z]
    ])
    Q_kxi[-2:, :3] = G_xi_R

    # Initialize landmark covariance
    W_k = np.diag([1000, 1000])
    P_aug = np.block([
        [P, np.zeros((P.shape[0], 2))],
        [np.zeros((2, P.shape[1])), W_k]
    ])

    # Update covariance matrix
    ekf_state['P'] = Q_kxi @ P_aug @ Q_kxi.T
...
def compute_rmse(estimated, ground_truth, timestamps_est, timestamps_gt):
    # Interpolate ground_truth to match timestamps_est
    interpolated_gt = np.zeros_like(estimated)
    for i in range(3):  # For x, y, phi
        interpolated_gt[:, i] = np.interp(
            timestamps_est, timestamps_gt, ground_truth[:, i]
        )

    errors = estimated - interpolated_gt
...
def plot_trajectory(estimated, ground_truth):
    ...
    plt.plot(estimated[:, 0], estimated[:, 1], label="Estimated", linestyle='--')
...
def main():
    ...
    predicted_trajectory = np.array(state_history['x'])
    timestamps_predicted = np.array(state_history['t'])
    timestamps_gps = gps[:, 0]

    rmse_value = compute_rmse(predicted_trajectory, gps[:, 1:], timestamps_predicted, timestamps_gps)

    # Plot trajectory
    plot_trajectory(predicted_trajectory, gps, timestamps)
